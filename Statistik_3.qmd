...
@fig-cor zeigt fünf verschiedene Situationen. Bei (a) liegt eine positive Korrelation vor, bei (b) eine negative und bei (c)–(e) keine Korrelation. Bei (e) erkennt man zwar visuell eine Beziehung (ein "Peak" in der Mittel, also eine unimodale Beziehung), aber das ist eben kein linearer Zusammenhang.

::: {#fig-cor layout-nrow=2}
...
Verschiedene Korrelationen (aus Quinn & Keough 2002)

:::



Bei der Pearson-Korrelation betrachtet man die beiden Parameter Kovarianz (reicht von −∞ bis +∞) und die Korrelation, welche die Kovarianz auf den Bereich von –1 bis +1 standardisiert. Pearsons Korrelationskoeffizient r ist der Schätzer für die Korrelation basierend auf der Stichprobe (@tbl-covcor).
...
: Definition von Kovarianz und Pearson Korrelation (griechische Buchstaben beziehen sich auf den Wert in der Grundgesamtheit, römische auf den Schätzwert basierend auf der Stichprobe) {#tbl-covcor}
...
- $\mathbf{\mathit{x}}$ : **unabhängige Variable** (*independent variable*), Prädiktorvariable (*predictor*)
- $\mathbf{\mathit{y}}$ : **abhängige Variable** (*dependent variable*), Antwortvariable (*response*)
...
- **Statistisches Modell**: $y_i = \beta_0 + \beta_1 x_i + \epsilon_i$, wobei $\epsilon_i$ das Residuum des $i$-ten Datenpunktes ist, d.h. seine vertikale Abweichung vom vorhergesagten Wert.
...
- $H_0$:$ \beta_1 = 0$ (Steigung [*slope*] der Grundgesamtheit ist Null)
...
Es mag vielleicht zunächst überraschen, aber ähnlich wie beim Vergleich von Mittelwerten zwischen kategorischen Ausprägungen kategorischer Variablen, liegt auch der linearen Regression eine **Varianzanalyse** zugrunde wie @tbl-anovalin zeigt. Die Bedeutung der verschiedenen Differenzen ist in Abbildung 2.7 visualisiert.

| Source of variation | SS | df | MS | Expected mean square |
|---|---|---|---|---|
| Regression | $\sum_{i=1}^{n} (\hat{y}_i - \bar{y})^2$ | $1$ | $\frac{\sum_{i=1}^{n} (\hat{y}_i - \bar{y})^2}{1}$ | $\sigma_{\varepsilon}^{2} + \beta_{1}^{2} \sum_{i=1}^{n} (X_{i} - \bar{X})^{2}$ |
| Residual | $\sum_{i=1}^{n} (y_i - \hat{y}_i)^2$ | $n - 2$ | $\frac{\sum_{i=1}^{n}(y_i-\hat{y}_i)^2}{n-2}$ | $\sigma_{\varepsilon}^{2}$ |
| Total | $\sum_{i=1}^{n} (y_i - \bar{y})^2$ | $n - 1$ |  |  |

: ANOVA-Tabelle für die einfache lineare Regression von Y auf X {#tbl-anovalin}
...
Wiederum ist die Teststatistik ein -ratio, nämlich $F = \frac{MS_{\text{Regression}}}{MS_{\text{Residual}}}$, wobei MS für die mittleren Quadratsummen steht, also die Quadratsummen (SS) geteilt durch die Freiheitsgrade (df). Wie oben unter der Varianzanalyse schon erwähnt, folgt *F* einer $t^2$-Verteilung.
...
Die **aov-Ansicht** liefert uns die oben besprochene ANOVA-Tabelle, einschliesslich der Signifikanz der Steigung (hier *p* < 0.001).
...
2. **Signifikanz (Verlässlichkeit des Ergebnisses)**: *p*-Wert der Steigung (`0.000145`), *nicht* der *p*-Wert des Achsenabschnittes (`3.25e-11`). Wird nach üblicher Konvention auf drei Nachkommastellen gerundet oder, wenn unter 0.001, dann als *p* < 0.001 angegeben.
3. **Effektgrösse und -richtung (unser eigentliches Ergebnis!)**: Im Falle einer linearen Regression ist das die Funktionsgleichung, die sich aus den Schätzungen der Koeffizienten ergibt. Hier `-0.26323`
4. **Erklärte Varianz (Relevanz des Ergebnisses)**: Wie viel der Gesamtvariabilität der Daten wird durch das Modell erklärt? Ob $R^2$ (`0.6831`) oder $R_{\text{adj.}}^2$ (`0.6588`) angegeben werden sollte, wird unterschiedlich gesehen, jedenfalls sollte man explizit sagen, was gemeint ist. $R^2$ ist übrigens der quadrierte Wert von Pearsons Korrelationskoeffizienten $r$.
5. **ggf. Wert der Teststatistik mit den Freiheitsgraden ("Zwischenergebnisse")**: $F_{1,13} = 28.08$

Ein adäquater Ergebnistext könnte daher wie folgt lauten:

::: {.callout}

Der Artenreichtum nahm hochsignifikant mit der Stickstoffdeposition ab (Funktionsgleichung: $S = 25.61 - 0.26 \times N,\; F_{1,13} = 28.08,\; p < 0.001,\; R^2 = 0.683$).
...
geom_point() +
geom_abline(intercept = lm_1$coefficients["(Intercept)"], 
            slope = lm_1$coefficients["N_deposition"], 
            color = "blue")
```
...
ggplot(df, aes(x = N_deposition, y = Species_richness)) +
  geom_point() +
  geom_abline(intercept = lm_1$coefficients[1], slope = lm_1$coefficients[2], color = "blue")
```
...
```{.r}
anova( lm(weight ~ age + sex + age:sex) 
lm(weight~age+sex))
```
...
| $y_i = \beta_1 x_i$ | `y ~ 0 + x`<br>`y ~ -1 + x`<br>`y ~ x - 1` | Simple linear regression model of y on x with intercept term excluded |
...
| $y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i1}^2$ | `y ~ 1 + x + I(x^2)`<br>`y ~ poly(x, 2)` | Second order polynomial regression of $y$ on $x$ |
| $y_{ij} = \mu + \alpha_i$ | `y ~ A` | Analysis of variance of y against a single factor A |
| $y_{ijk} = \mu + \alpha_i + \beta_j + \alpha \beta_{ij}$ | `y ~ A + B + A:B`<br>`y ~ A*B` | Fully factorial analysis of variance of y against A and B |
| $y_{ijk} = \mu + \alpha_i + \beta_j$ | `y ~ A*B - A:B` | Fully factorial analysis of variance of y against A and B without the interaction term |
| $y_{ijk} = \mu + \alpha_i + \eta_{j(i)}$ | `y ~ B %in% A`<br>`y ~ A/B` | Nested analysis of variance of y against A and B nested within A |
| $y_{ij} = \mu + \alpha_i + \beta(x_{ij} - \overline{x})$ | `y ~ A*x`<br>`y ~ A/x` | Analysis of covariance of y on $x$ at each level of A |

: Übersicht über die Vielzahl an linearen Modellen (Befehle lm, aov) in R möglich ist. Man beachte, dass in der R-Syntax «/» und «*» nicht für Divisionen oder Multiplikationen steht, sondern für Schachtelungen bzw. den Einschluss von Interaktionen. {#tbl-statmodR}

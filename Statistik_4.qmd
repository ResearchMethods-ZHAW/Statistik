...
- Das konventionelle Gütemass $R^2$ funktioniert ebenfalls nicht. Statt
    dessen kann man die Modellgüte mit einem **Pseudo-$R^2$** ausdrücken:
...
\\
y &= \frac{\exp(b_0 + b_1 x)}{1+ \exp(b_0+b_1x)}
\end{align}

Oder mit den Werten in unserem Fall:

$$
y = \frac{\exp(-5.47 + 0.28 x)}{1+ \exp(-5.47 + 0.28 x)}
$$
